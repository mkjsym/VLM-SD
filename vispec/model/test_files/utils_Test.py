...
def initialize_tree(
    ...
    **kwargs,
    ...
    attentions, orig, hidden_states = model(
        input_ids,
        past_key_values=past_key_values,
        output_orig=True,
    ...
    # Image Token Compression (AirCache)
    if image_mask is not None and attentions is not None:
        device = input_ids.device
        last_attn = attentions[-1] # (batch, num_heads, q_len, k_len)
        avg_attn = last_attn.mean(dim=1).squeeze(0) # 헤드 평균: (q_len, k_len)

        # 인덱스 추출
        img_indices = torch.where(image_mask[0])[0]
        text_indices = torch.where(~image_mask[0])[0]

        # STEP 1: Instruction 토큰 중 Key Text Token 선정
        # 마지막 토큰(-1)이 전체 텍스트 토큰 중 어디를 보는지 확인
        text_attn_from_last = avg_attn[-1, text_indices]

        max_text_attn = torch.max(text_attn_from_last)
        text_threshold_val = max_text_attn * text_threshold
        text_mask = text_attn_from_last >= text_threshold_val
        key_text_indices = text_indices[text_mask]
        if len(key_text_indices) == 0:
            _, top_1_idx = torch.topk(text_attn_from_last, 1)
            key_text_indices = text_indices[top_1_idx]

        # _, top_text_sub_idx = torch.topk(text_attn_from_last, min(top_k_text, len(text_indices)))
        # key_text_indices = text_indices[top_text_sub_idx]

        # STEP 2: Key Text Token들과 Image Token들 간의 Importance Score 측정
        # [Key Text 토큰들]이 [이미지 토큰들]에 보내는 어텐션의 합산
        # shape: (len(key_text_indices), len(img_indices))
        text_to_img_attn = avg_attn[key_text_indices][:, img_indices]
        importance_score = text_to_img_attn.sum(dim=0) # 이미지 토큰별 중요도 합산

        # STEP 3: Importance Score 기준 Top-K 이미지 토큰 선별
        _, top_img_sub_idx = torch.topk(importance_score, min(top_k_image, len(img_indices)))
        selected_img_indices = img_indices[top_img_sub_idx]

        # 보존할 전체 인덱스 결정 (텍스트 전체 + 선택된 이미지)
        keep_indices = torch.cat([text_indices, selected_img_indices]).sort()[0]

        # STEP 4: 데이터 압축 (Hidden States & KV Cache)
        # Hidden States 압축 (seq_len 차원 슬라이싱)
        hidden_states = hidden_states[:, keep_indices, :]

        # KV Cache 압축 (각 레이어의 key, value 슬라이싱)
        # past_key_values 구조에 따라 루프 돌며 처리 (일반적으로 tuple of tuples)
        new_past_key_values = []
        ...
        print(f"Compression Complete: {len(img_indices)} -> {len(selected_img_indices)} image tokens")

    # 모델의 KV Cache 상태 업데이트 (중요: initialize_past_key_values와 연동 필요)
    if image_mask is not None and keep_indices is not None:
        # 1. 유지할 토큰 개수 계산
        new_len = len(keep_indices)

        # 2. past_key_values_data 직접 수정 (In-place)
        # self.past_key_values_data: [num_layers, 2, batch, num_heads, max_seq_len, head_dim]
        # 주의: 모델 구조에 따라 차원 순서가 다를 수 있으니 확인 필요
        
        with torch.no_grad():
            # 모든 레이어의 K, V 데이터를 보존할 인덱스 데이터로 덮어쓰기
            # .copy_()를 사용하여 기존 메모리 주소를 유지하면서 값만 변경합니다.
            model.past_key_values_data[:, :, :, :, :new_len, :].copy_(
                model.past_key_values_data[:, :, :, :, keep_indices, :]
            )
            
            # 3. current_length_data 업데이트 (포인터 동기화)
            # 이 포인터가 줄어들어야 다음 토큰이 압축된 이미지 바로 뒤에 저장됩니다.
            model.current_length_data.zero_()
            model.current_length_data += new_len

        # 4. Hidden States 동기화
        # topK_genrate 단계로 넘어갈 hidden_states도 동일한 인덱스로 슬라이싱
        hidden_states = hidden_states[:, keep_indices, :]
        
        # 5. (중요) input_ids와 image_mask 업데이트
        # 이후 로직에서 사용될 input_ids도 압축된 결과와 일치해야 합니다.
        input_ids = input_ids[:, keep_indices]
        
        # 만약 spec_layer가 image_mask를 참조한다면 이것도 업데이트
        if image_mask is not None:
            new_image_mask = image_mask[:, keep_indices]
        else:
            new_image_mask = None

        print(f"KV Cache Sync Complete. New Cache Length: {model.current_length_data.item()}")
